...
begin
  try
    ...
    try
      DBBeginTrans;
      vsSQL := 'SELECT Cli_Codigo FROM Lancamentos WHERE Cli_Codigo = '+ovCE_Codigo.Text+' LIMIT 1';
      ExecSQL(vsSQL, oCDS);
      if not oCDS.IsEmpty then
      begin
        p_MsgAviso('Não é possivel excluir este cadastro pois existem lançamentos para este registro.');
        Exit;
        {if not f_MsgConfirma('Existe lançamentos para este cliente. Deseja realmente remover este cadastro?') then
          Exit;
        vsSQL := 'DELETE FROM TitulosReceber WHERE Cli_Codigo = '+ovCE_Codigo.Text;
        ExecSQL(vsSQL);
        vsSQL := 'DELETE FROM Lancamentos WHERE Cli_Codigo = '+ovCE_Codigo.Text;
        ExecSQL(vsSQL);}
      end;
